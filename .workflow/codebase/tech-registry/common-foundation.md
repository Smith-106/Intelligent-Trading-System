...
- `quantflow/common/monitoring_sink.py` — **ISS-019 L6 解耦 seam**: `MonitoringSink` (`@runtime_checkable` Protocol, 16 methods: start/record_signal/record_bar_latency/record_signal_latency/record_portfolio/record_risk_event/record_kill_switch_activation/record_kill_switch_step_failure/record_order_total/record_order_filled/record_order_latency/record_gateway_connected/record_gateway_disconnect/record_gateway_reconnect/record_order_timed_out/send_alert — ISS-019 基线 + ISS-044 +6 record_order/risk/kill_switch + ISS-011 +4 gateway/timeout) + `NullMonitoringSink` (zero-cost no-op default, all 16 mirrored, used by backtest/tests). Lower layers depend on this Protocol instead of importing `quantflow.monitoring.*`.
...
- **ISS-012 config-sourced sizing thresholds (commit `8ffd612`)**: `RiskConfig.fixed_pct` (L84, `=0.10`) + `min_order_notional` (L87, `=10.0`) added — previously hardcoded fallback in `PositionSizer`. `fee_rate` reuses `ExecutionConfig.taker_fee` (L97, `=0.001`, D3 single-source-of-truth — no new ExecutionConfig field). Defaults byte-for-byte match the prior hardcoded values to preserve backtest baseline. `strategy/engine.py:116` `PositionSizer(...)` constructor threads the 3 params.
- **ISS-019 L6 解耦 seam (new)**: `monitoring_sink.py` defines the `MonitoringSink` Protocol + `NullMonitoringSink` no-op default. Lower layers (L3 `strategy/engine`, L4 `signal/risk_engine`, L5 `execution/engine`+`kill_switch` + ISS-011 `okx_gateway`+`order_manager`) take `monitoring_sink: MonitoringSink | None = None` and default to `NullMonitoringSink`, eliminating direct `quantflow.monitoring.*` imports (closes arch-013 audit-evasion lazy-import). The L6 impl `monitoring/sink.py` `DefaultMonitoringSink` is injected by high-level callers only. Protocol grew 12→16 methods: ISS-011 +4 (record_gateway_connected/disconnect/reconnect + record_order_timed_out).
