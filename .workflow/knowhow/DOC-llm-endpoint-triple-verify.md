...
summary: 1. **Endpoint 可达**：HTTP 200 + model echo（确认端点返回预期模型名） 2. **Key 有效**：usage tokens 非零（确认 API key 有配额） 3. **Env wiring 正确**：3/3 assertions（env 三要素齐全 / 无 key 时 None 降级 / config 优先级覆盖）
