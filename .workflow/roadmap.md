...
- [~] **P2.2 FinBERT→FinGPT 升级**（依赖 P2.1）：单卡 RTX 3090/$17.25 微调路径（medium-high，2-1 vote，自报基准）。**环境解封，推理路径验证中**（2026-08-06）：CPU 版 torch 2.13.0 + transformers 5.14.1 已装；接口层已交付（生成式 LM 分支 + 3 测试）+ `scripts/finetune_fingpt.py`（设备检测/CPU 警告/7B 需 GPU）+ `scripts/smoke_fingpt_generative.py`；tiny-gpt2 真实推理冒烟 **PASS**（CausalLM 加载→generate→关键词解析→不可解析 NaN sentinel 全链路）；金融微调模型（rezacsedu GPT-2 124M）权重下载中（网络 ~70KB/s），下载完成后补标签解析验证
