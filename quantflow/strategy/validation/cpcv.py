...
def split_cpcv(
    ...
    groups = [np.arange(i * group_size, min((i + 1) * group_size, n_bars))
    ...
    logger.info("CPCV: %d groups × %d test = %d paths (embargo=%d bars)",
                n_groups, n_test_groups, len(splits), embargo_bars)
    return splits
...
def cpcv_backtest(
    ...
    pbo = float(np.mean(is_sharpes > oos_sharpes))

    # OOS efficiency
    oos_efficiency = float(np.mean(oos_sharpes) / max(np.mean(is_sharpes), 1e-6))
    ...
    result = {
        ...
        "oos_sharpe_min": float(np.min(oos_sharpes)),
        "path_results": path_results,
        "passed": pbo < 0.5,
    }

    logger.info("CPCV result: PBO=%.3f, OOS eff=%.3f, OOS Sharpe mean=%.3f±%.3f, passed=%s",
                pbo, oos_efficiency, result["oos_sharpe_mean"], result["oos_sharpe_std"],
                result["passed"])
    return result
