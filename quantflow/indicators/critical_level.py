...
class CriticalLevelType(StrEnum):
    W1_ORIGIN = "w1_origin"
    W1_PEAK = "w1_peak"
    W3_PEAK = "w3_peak"
    W4_LOW = "w4_low"
...
class CriticalLevelDetector(FactorBase):
    ...
    def compute(self, df: pd.DataFrame, **params: Any) -> pd.Series:
        ...
        hard_levels = [level.price for level in levels.levels if level.severity == "hard"]
        if not hard_levels:
            return pd.Series(float("nan"), index=df.index)
    ...
    def _build_bull_scenario(
        ...
        for level in levels:
            if level.level_type == CriticalLevelType.W1_PEAK:
                trigger = level
                break
    ...
    def _build_bear_scenario(
        ...
        for level in levels:
            if level.level_type == CriticalLevelType.W1_ORIGIN:
                trigger = level
                break
