...
from quantflow.common.models import OrderRequest, OrderSide, OrderType, OrderStatus
        ...
        def update_orders(thread_id, order_ids):
            try:
                for oid in order_ids:
                    manager.update(oid, OrderStatus.FILLED, filled_quantity=0.5, filled_price=50000.0)
            except Exception as e:
                errors.append(f"Thread {thread_id}: {str(e)}")
