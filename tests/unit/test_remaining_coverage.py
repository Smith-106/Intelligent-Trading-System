...
class TestWaveIdentifierImpulseThenCorrective:
    ...
    def test_identify_returns_corrective_when_no_impulse(self):
        ...
        from quantflow.indicators.zigzag import PivotPoint, PivotDirection, PivotSequence
        wid = WaveIdentifier()
        # Create pivots that don't form a valid impulse but might form a corrective
        pivots_seq = PivotSequence(pivots=[
...
class TestHistoryDedupeKey:
    ...
    def test_list_with_dedupe(self, tmp_path):
        ...
        store.append_research_run({"request": {"strategy": "trend", "symbol": "BTC/USDT"}, "result": {"r": 1}})
        store.append_research_run({"request": {"strategy": "trend", "symbol": "ETH/USDT"}, "result": {"r": 2}})
        store.append_research_run({"request": {"strategy": "mr", "symbol": "BTC/USDT"}, "result": {"r": 3}})
...
class TestServiceDownloadDataErrors:
    ...
    async def test_download_empty_after_clean(self):
        ...
        mock_fetcher = MagicMock()
        raw_frame = pd.DataFrame({"timestamp": [1], "open": [100], "high": [101], "low": [99], "close": [100], "volume": [1000]})
...
class TestServiceTagDataSourceNoFiles:
    ...
    def test_tag_no_parquet_files(self):
        ...
        req = DataSourceTagRequest(
            symbol="BTC/USDT", data_source="okx",
            config_path="quantflow/config/default.yaml",
        )
        ...
        with patch("quantflow.web.service.load_config") as mock_load, \
            ...
            with patch("quantflow.web.service._open_station_store", return_value=mock_store), \
                ...
                result = service.monitoring_snapshot(
                    session_snapshot=None, session_history=[], session_events=[],
                )
    ...
    def test_symbol_source_unknown_in_demo_seeded_mode(self):
        ...
        overview_data = {
            ...
            "data": {
                ...
                "symbols": [{"symbol": "BTC/USDT", "data_source": "unknown", "files": 1, "date_range": [1700000000000, 1700003600000], "source_breakdown": {}}],
                "mode": "demo-seeded",
...
class TestTrendFollowingRSIAdaptiveProfitOversold:
    ...
    def test_rsi_oversold_at_entry(self):
        ...
        strategy = TrendFollowingStrategy(params={
            "rsi_adaptive_profit": True, "min_conditions": 1, "rsi_period": 5,
        })
        n = 120
...
class TestVolatilityBreakoutBbMiddleZero:
    ...
    def test_latest_signal_zero_close_prices(self):
        ...
        strategy = VolatilityBreakoutStrategy(params={"bb_period": 3, "atr_period": 2, "keltner_ema_period": 3, "keltner_atr_period": 2})
        # Set up internal state manually so _runtime_state_is_current() is False
...
class TestMlEnsembleNoOosPredictions:
    ...
    def test_train_model_all_nan_oos(self):
        ...
        with patch("quantflow.strategy.templates.ml_ensemble._positive_class_probability", side_effect=nan_proba_side_effect), \
...
class TestDataInitGetAttr:
    ...
    def test_getattr_nonexistent_raises(self):
        ...
        with pytest.raises(AttributeError, match="has no attribute"):
            quantflow.data.NonExistentAttr
...
class TestRegimeDetectAtrFallback:
    ...
    def test_update_short_atr_fallback(self):
        ...
        for h, l, c in [(100, 99, 100), (101, 100, 101), (102, 101, 102), (103, 102, 103)]:
            regime = detector.update(h, l, c)
...
class TestWaveIdentifierDiagonalBearish:
    ...
    def test_bearish_diagonal_exception(self):
        ...
        pivots_seq = PivotSequence(pivots=[
            ...
            PivotPoint(index=5,  price=100.0, direction=PivotDirection.LOW,  confidence=1.0),  # W1: -100
            PivotPoint(index=10, price=140.0, direction=PivotDirection.HIGH, confidence=1.0),  # W2: +40
            PivotPoint(index=15, price=70.0,  direction=PivotDirection.LOW,  confidence=1.0),  # W3: -70
            PivotPoint(index=20, price=105.0, direction=PivotDirection.HIGH, confidence=1.0),  # W4: +35
            PivotPoint(index=25, price=80.0,  direction=PivotDirection.LOW,  confidence=1.0),  # W5: -25
        ])
        result = wid.identify(pivots_seq, mode=AnalysisMode.RETROSPECTIVE)
        ...
        pivots_seq2 = PivotSequence(pivots=[
            PivotPoint(index=0,  price=200.0, direction=PivotDirection.HIGH, confidence=1.0),
        ...
        iron_law = wid._validate_iron_laws(result2.waves, AnalysisMode.RETROSPECTIVE, is_bullish=False)
    ...
    def test_artifact_request_no_top_level_request(self):
        ...
        overview_data = {
            ...
            "data": {
                "parquet_dir": "/tmp/test",
                "duckdb_path": "/tmp/test.duckdb",
                "symbols": [{"symbol": "BTC/USDT", "data_source": "okx", "files": 1, "date_range": [1700000000000, 1700003600000], "source_breakdown": {}}],
                "mode": "market",
                "source_context": {"message": "live market data"},
            },
        }
...
class TestServiceMonitoringHealthAccentDowngrade:
        ...
        return {
            ...
            "data": {
                ...
                "symbols": [{"symbol": "BTC/USDT", "data_source": "okx", "files": 1,
                            "date_range": [1700000000000, 1700003600000], "source_breakdown": {"okx": 1}}],
            },
    ...
    def test_external_unavailable_started_downgrades_accent(self):
        ...
        with patch.object(service, "overview", return_value=self._make_overview_market()), \
             patch("quantflow.web.service.metrics_registry_snapshot", return_value={"values": {}, "available": True}), \
             patch("quantflow.web.service.metrics_server_status", return_value={"attempted": True, "started": True, "started_in_process": True}), \
...
class TestAppRunStation:
    ...
    def test_run_station_calls_run_app(self):
        ...
        with patch("quantflow.web.app.web") as mock_web:
