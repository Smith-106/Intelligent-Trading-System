...
class TestResolveFrameDataSource:
    ...
    def test_empty_frame(self):
        ...
        df = pd.DataFrame()
        source, _breakdown = _resolve_frame_data_source(df)
...
class TestBuildDemoFrame:
    ...
    def test_large_range_downsampled(self):
        ...
        df = _build_demo_frame(
            "BTC/USDT", start="2020-01-01", end="2026-01-01", bars=100, timeframe="1h"
        )
        assert len(df) == 100
...
class TestValidationSummary:
    ...
    def test_cpcv_method(self):
        ...
        payload = {
            ...
            "result": {
                ...
                "oos_efficiency": 0.7,
                "n_paths": 15,
                "signal_quality": {"precision": 0.6, "recall": 0.5, "n_signals": 10},
            },
            "signals": {"entries": 5, "exits": 3, "bars": 100},
            "backtest": {},
        }
    ...
    def test_wfo_method_mixed(self):
        ...
        payload = {
            "method": "wfo",
            "result": {
                "rolling": {"passed": True, "decision": "PASS"},
                "anchored": {"passed": False, "decision": "FAIL"},
            },
            "signals": {"entries": 5, "exits": 3, "bars": 100},
            "backtest": {},
        }
    ...
    def test_wfo_method_both_fail(self):
        ...
        payload = {
            "method": "wfo",
            "result": {
                "rolling": {"passed": False, "oos_sharpe_mean": 0.2, "decision": "FAIL"},
                "anchored": {"passed": False, "oos_sharpe_mean": 0.1, "decision": "FAIL"},
            },
            "signals": {"entries": 5, "exits": 3, "bars": 100},
            "backtest": {},
        }
...
class TestResultPayload:
    def test_basic(self):
        ...
        pd.Series(100.0 + np.random.default_rng(42).normal(0, 1, 50).cumsum(), index=dates)
...
class TestChartPayload:
    ...
    def test_with_timeframe_column(self):
        ...
        result = BacktestResult(
            ...
            profit_factor=1.8,
            num_trades=5,
            equity_curve=pd.Series(10000.0, index=dates),
            drawdown_curve=pd.Series(0.0, index=dates),
        )
        payload = _chart_payload(frame, entries, exits, result)
...
class TestStationServiceStrategies:
    def test_strategies_returns_list(self):
        ...
        with patch(
            "quantflow.web.service.list_strategy_summaries", return_value=[{"name": "test"}]
        ):
            service = StationService(history_store=StationHistoryStore())
...
class TestStationServiceResearchHistory:
    def test_research_history(self, tmp_path):
        ...
        store.append_research_run(
            {"request": {"strategy": "test"}, "result": {"total_return": 0.1}}
        )
        service = StationService(history_store=store)
        ...
        store.append_validation_run(
            {
                ...
                "payload": {
                    "method": "gate",
                    "result": {"decision": "NO-GO", "checks": {"cpcv": {"passed": False}}},
                    "signals": {"entries": 5, "exits": 3, "bars": 100},
                    "backtest": {},
                },
                "summary": {
...
class TestQuerySymbolFrame:
    ...
    def test_frame_with_datetime_column(self):
        ...
        with patch(
            ...
            _frame, source = _query_symbol_frame(mock_store, "BTC/USDT")
            assert source == "okx"
    ...
    def test_frame_with_timestamp_column_and_filter(self):
        ...
        with patch(
            ...
            frame, _source = _query_symbol_frame(
                mock_store,
                "BTC/USDT",
                start="2023-11-15",
                end="2023-11-30",
...
class TestStationServiceResearch:
    def test_research_basic(self):
        from quantflow.web.history import StationHistoryStore
        from quantflow.web.service import ResearchRequest, StationService
